...
export default function SupplierEditProfileScreen({
	user: initialUser,
	language,
	onBack,
	onSave
	...
	const t = getTranslations('supplier', 'profile', language ?? 'en')
	...
	const pickImage = async () => {
		try {
			...
			if (status !== 'granted') {
				toastShow(commonT.error || 'Error', t.permissionDenied || 'Permission to access camera roll is required')
				return
			}
			...
			if (!result.canceled && result.assets[0]) {
				...
				try {
					if (base64) {
						...
						toastShow(commonT.error || 'Error', t.imageConversionFailed || 'Failed to process image')
			...
			toastShow(commonT.error || 'Error', t.imagePickFailed || 'Failed to pick image')
		}
	}
	...
	const isFormValid = firstName.trim() !== '' && lastName.trim() !== '' && email.trim() !== '' && emailValid === true
	...
	const handleSave = async () => {
		...
		setLoading(true)
		try {
			// Update user profile (first_name, last_name, email, profile_image)
			// Note: profile_image might not be supported for suppliers, but we'll try
			...
			// Only include profile_image if it's set (suppliers might not support it)
			if (profileImage !== null) {
				updateData.profile_image = profileImage || undefined
			}

			const updatedUser = await user.updateProfile(updateData)

			toastShow(commonT.success || 'Success', t.profileUpdated || 'Profile updated successfully')
			if (onSave) {
				onSave(updatedUser)
			}
			onBack()
		} catch (e: any) {
			const msg = e?.message || e?.body?.detail || t.updateFailed || 'Failed to update profile'
	...
	return (
		...
		>
			...
			<View style={styles.header}>
				...
				<Text style={styles.headerText}>{t.editProfile || 'Edit Profile'}</Text>
			</View>

			<View style={styles.body}>
				<View style={styles.bodyContent}>
					<Text style={{ fontWeight: '700', marginBottom: 8, fontSize: 16, color: '#111827' }}>{t.personalInformation || 'Personal Information'}</Text>
					...
					<View style={{ alignItems: 'center', marginBottom: 24 }}>
						...
						>
							<Text style={{ color: '#2563eb', fontSize: 14, fontWeight: '600' }}>
								{profileImage || profileImageUri ? (t.changePhoto || 'Change Photo') : (t.uploadPhoto || 'Upload Photo')}
							</Text>
						</TouchableOpacity>
					</View>
					...
					>
						{loading ? <ActivityIndicator color="#fff" /> : <Text style={styles.continueButtonText}>{t.save || 'Save Changes'}</Text>}
					</TouchableOpacity>
				</View>
			</View>
		</SafeAreaView>
