...
export default function SupplierComplaintDetailScreen({
	...
	onBack?: () => void
	onOpenChat?: (orderId?: string | null) => void
	language?: Language
}) {
	...
	useEffect(() => {
		...
		const load = async () => {
			...
			try {
				...
				let orderData = null
				try {
					orderData = await orders.fetchOrderById(String(found.order_id || found.orderId))
				} catch (e) {}
		...
		return (
			<SafeAreaView style={{ flex: 1 }}>
				<View style={styles.header}>
					...
					<Text style={{ fontSize: 18, fontWeight: '700' }}>{t.complaintDetails}</Text>
					<View style={{ width: 36 }} />
				</View>
			...
			<ScrollView contentContainerStyle={{ padding: 16 }}>
				...
				<View style={{ marginTop: 24 }}>
					...
					<TouchableOpacity
						...
						style={[
							{ backgroundColor: '#059669', padding: 14, borderRadius: 8, alignItems: 'center', marginBottom: 12 },
							(resolved || actionLocked) && { opacity: 0.6 }
						]}
					>
					...
					<TouchableOpacity
						onPress={async () => {
							...
							try {
								...
								setActionLocked(true)
								setEscalating(true)
								await complaints.updateComplaintStatus(c.id, 'escalated', 'Complaint escalated to manager')
								setEscalated(true)
						...
						style={[
							{ backgroundColor: '#f97316', padding: 14, borderRadius: 8, alignItems: 'center', marginBottom: 12 },
							(escalated || actionLocked) && { opacity: 0.6 }
						]}
					>
					...
					<TouchableOpacity
						onPress={() => {
							if (onOpenChat) onOpenChat(c.orderId)
						}}
						style={{ backgroundColor: '#fff', borderWidth: 1, borderColor: '#e5e7eb', padding: 14, borderRadius: 8, alignItems: 'center' }}
					>
