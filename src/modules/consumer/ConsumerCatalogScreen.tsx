...
export default function ConsumerCatalogScreen({ language, navigateTo, onProductSelect }: ConsumerCatalogScreenProps) {
	...
	const acceptedSuppliers = (suppliers || []).filter((s: any) => String(s.status).toLowerCase() === LINK_STATUS.ACCEPTED.toLowerCase())
	...
	// Note: Search is now handled server-side via backend catalog endpoint
		...
		<SafeAreaView style={{ flex: 1, backgroundColor: '#fff' }}>
			<View style={styles.header}>
				<View style={{ flexDirection: 'row', alignItems: 'center' }}>
					<TouchableOpacity
						onPress={() => navigateTo('consumer-home')}
			...
			{suppliersLoading ? (
				...
				<View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', padding: 24 }}>
					<Text style={{ color: '#6b7280', textAlign: 'center' }}>
						{t.noAcceptedLinks || "You don't have any accepted supplier links yet. Request a link from suppliers to view their catalog."}
					</Text>
					<TouchableOpacity
						onPress={() => navigateTo('linked-suppliers')}
						style={{ marginTop: 12, padding: 12, backgroundColor: '#2563eb', borderRadius: 8 }}
					>
						<Text style={{ color: '#fff' }}>{t.requestLink || 'Request Link'}</Text>
					</TouchableOpacity>
				</View>
				...
				<View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', padding: 24 }}>
					<Feather
						...
						color="#cbd5e1"
					/>
					<Text style={{ color: '#6b7280', marginTop: 12, fontSize: 16, fontWeight: '600' }}>{t.noProductsFound || 'No products found'}</Text>
					<Text style={{ color: '#9ca3af', marginTop: 4, textAlign: 'center' }}>{t.tryDifferentSearch || 'Try a different search term'}</Text>
				</View>
			) : (
				<FlatList
					data={filteredItems}
