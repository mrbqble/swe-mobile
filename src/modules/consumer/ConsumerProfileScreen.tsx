...
export default function ConsumerProfileScreen({ user, language, setLanguage, onLogout, onBack }: { user: User; language: 'en' | 'ru'; setLanguage: (l: 'en' | 'ru') => void; onLogout: () => void; onBack?: () => void }) {
  const t = {
    en: { profile: 'Profile', personalInformation: 'Personal Information', name: 'Name', organization: 'Organization', email: 'Email', settings: 'Settings', languageLabel: 'Language', logout: 'Logout', english: 'English', russian: 'Русский' },
    ru: { profile: 'Профиль', personalInformation: 'Личная информация', name: 'Имя', organization: 'Организация', email: 'Эл. почта', settings: 'Настройки', languageLabel: 'Язык', logout: 'Выйти', english: 'English', russian: 'Русский' }
  } as any;
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#fff' }}>
      ...
      <View style={{ alignItems: 'center', paddingTop: 24 }}>
        <View style={styles.avatar}><Text style={{ color: '#2563eb', fontSize: 28 }}>👤</Text></View>
        <Text style={{ marginTop: 12, fontWeight: '700', fontSize: 16 }}>{user.name}</Text>
        <Text style={{ color: '#6b7280', marginTop: 4 }}>{user.org}</Text>
      </View>

      <View style={{ padding: 16 }}>
        <Text style={{ fontWeight: '700', marginBottom: 8 }}>{L.personalInformation}</Text>
        <View style={styles.infoCard}><Text style={{ color: '#6b7280' }}>{L.name}</Text><Text style={{ marginTop: 6, fontWeight: '600' }}>{user.name}</Text></View>
        <View style={styles.infoCard}><Text style={{ color: '#6b7280' }}>{L.organization}</Text><Text style={{ marginTop: 6, fontWeight: '600' }}>{user.org}</Text></View>
        <View style={styles.infoCard}><Text style={{ color: '#6b7280' }}>{L.email}</Text><Text style={{ marginTop: 6, fontWeight: '600' }}>{user.email}</Text></View>
        ...
        <View style={[styles.infoCard, { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }]}> 
          ...
          <View>
            <TouchableOpacity onPress={() => setLanguage(language === 'en' ? 'ru' : 'en')} style={styles.langBtn}>
